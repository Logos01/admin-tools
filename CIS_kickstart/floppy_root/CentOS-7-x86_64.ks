...
rootpw --iscrypted $1$SkiSJlUp$w8j0MLPMQd1DKoO1jNkE41
#rootpw --iscrypted $5$9xXy18JLl44d2I$bKMZbRXJtF8A2TtS4nDmTm8tVU4uRoWbzWKI00q7SsD
...
# Company will use 'old-style' iptables service.
...
$(grep -q '/dev/shm' /mnt/sysimage/etc/fstab)
shm_present=$?
if [ "${shm_present}" == "0" ] ; then
    sed -i '/\/dev\/shm/s/defaults/nodev,nosuid,noexec,auto,nouser,async,relatime/' /mnt/sysimage/etc/fstab
else
    echo "" >> /mnt/sysimage/etc/fstab
    echo "tmpfs                   /dev/shm                tmpfs   defaults,nodev,nosuid,noexec,auto,nouser,async,relatime        0 0" >> /mnt/sysimage/etc/fstab
fi
...
cat /mnt/floppy/sysctl.conf >> /mnt/sysimage/etc/sysctl.conf
...
cp /tmp/floppy/password-auth-ac /mnt/sysimage/etc/pam.d/password-auth-ac
...
rm -f /mnt/sysimage/etc/{issue{,.net},motd}
...
echo '''
#####################################################################################
# CIS_CentOS_Linux_7_Benchmark_v1.1.0 compliant Kickstart profile for Company, LLC
# Created 2015-09-18 by Logos01 <Logos01 @ irc.freenode.net>
...
#     Oracle application/DB servers require X11.
#     3.6 -> NTP *IS* complied with.
#     CentrifyDC is currently used for centralized auth on many servers.
#
# - CIS 3.16 -> Configure Mail Transfer Agent for Local-Only Mode
#     Company use system emails for multiple business processes
#     This includes alerting/monitoring. Disregarded for that reason.
#
# - CIS 4.3 -> Wireless Networking
#     Company Servers do not have wireless networking hardware.
#
# - CIS 4.4 -> Configure IPv6
#     Company Servers will have IPv6 fully disabled.
...
#     Company security stance is in concurrence with NSA Guidance:
...
#     The need for direct root login on systems is present due to Company practices.
#     As such, only pubkey-based authentication will be permitted.
#     A pubkey rotation standard will be implemented via configuration management.
#     Additionally, pubkey-based root access is permitted only from authorized hosts.
#
# - CIS 6.3.1 -> Upgrade Password Hashing Algorithm to SHA-512
#     Enabled via kickstart.
#
